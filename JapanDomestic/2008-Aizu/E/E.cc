...
struct line {
    P s, e;
    double h;
    line(P s, P e, double h = 0.0) : s(s), e(e), h(h) {};
};

double cross(P p, P q) { return imag(conj(p) * q); }

bool isCrossed(const line& e1, const line& e2)
{
    P A = e1.e - e1.s, B = e2.e - e2.s, S = e2.s - e1.s;
    double f = cross(A, B);
...
int main(void)
{
    for (int N; cin >> N, N; ) {
        ...
        line course = line(P(sx, sy), P(ex, ey));
        ...
        for (int i = 0; i < N; ++i) {
            ...
            cin >> x1 >> y1 >> x2 >> y2 >> h;
            obs.push_back(line(P(x1, y1), P(x1, y2), h));
            obs.push_back(line(P(x1, y1), P(x2, y1), h));
            obs.push_back(line(P(x2, y2), P(x1, y2), h));
            obs.push_back(line(P(x2, y2), P(x2, y1), h));
